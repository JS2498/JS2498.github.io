...
For any reinforcement learning (RL) algorithm, *prediction (policy evaluation)* and *control (policy improvement)* are the two main steps. The state-value corresponding to each state $s$ for a policy $\pi$ is given by,

$$ v_{\pi}(s) = \mathbb{E}_{\pi}\left[\sum_{t=t_0}^T r_t|(s_{t_0}=s)\right],$$

where $T$ is the episode length. In Monte-Carlo method, the state-value, $v_{\pi}(s)$ is estimated by averaging the returns over episodes where, $G(s) = \sum_{t=t_0}^T r_t|(s_{t_0}=s)$ is the return obtained in a episode following policy $\pi$ in which the state $s$ has been visited along the trajectory.
