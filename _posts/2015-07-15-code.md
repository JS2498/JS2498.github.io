...
The First-Vist MC agent was not able to learn a optimal policy to reach the destination even after playing $10^{6}$ games. Increasing the number of episodes to $500/800$ also didn't work. This may be due to the fact that the agent only receives $-1$ as the reward till it reaches the destination. Hence the agent is not able to find a policy that would reslut in reaching the destination. Hence to overcome this an additional reward was given whenever the agent took an action that resultion in the next position being greater than $0.5$. Since the car has to move back and forth to reach the destination, the actions that resulted in car position being less than $0.5$ were not penalized. An extra reward of $100$ was given whenver the agent reached the destination. It was observed that under this reward reshaping the agent was able to find a optimal policy resulting in the car reaching the destination. The changes made to the reward structure was:
